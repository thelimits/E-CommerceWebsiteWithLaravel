            ...
            <div class="row g-0">
              <div class="col-md-4">
                <div class="card image ml-3 mt-3" >
                    @if ($datas->Stock <= 0)
                        <img class="mt-5" src="{{ asset('Asset/PngItem_2532316.png') }}" style="width: 228px; z-index: 1; position: absolute">
                    @endif
                    <img title="{{ 'Gambar '.$datas->Nama_Barang }}" style="height: 250px" src={{ url('storage/app/public/'.$datas->Image) }} class="card-img-top" alt="{{ $datas->Image }}">
                </div>
              </div>
              <div class="col-md-8">
                ...
                @endif
                    ...
                    @else
                        ...
                        <form action="/Home/{{ auth()->user()->role }}/Detail/Barang/{{ $datas->id }}/Cart" method="POST" style="display: flex; align-items: center; justify-content: space-between">
                            @csrf
                            <input min="1" max="{{ $datas->Stock }}" name="Quantity" style="height: 35px; border: 1px solid rgb(210, 210, 210); text-align: center" type="number" id="exampleInputQuantity">
                            <button type="submit" @if ($datas->Stock == 0)
                                disabled
                            @endif class="btn btn-success ml-3" style="width: 60%"> Add To Cart </button>
                        </form>
                        @error('Quantity')
                            <p style="color: red">
                                {{ $message }}
                        ...
                        @enderror
                        <a style="width: 200px" href="/Home/{{ auth()->user()->role }}/Home" class="btn btn-danger mt-2"> Back </a>
                    @endif
                </div>
              </div>
            </div>
